...
async function main() {
  ...
  // Deploy Token 1
  let le = await Token.deploy('Le Token', 'LE', '1000000') // 1 million tokens
  await le.deployed()
  console.log(`Le Token deployed to: ${le.address}\n`)
  ...
  const amm = await AMM.deploy(le.address, usd.address)
